...
class ModelPredictionWorker(QRunnable):
    ...
    def run(self):
        ...
        try:
            ...
            # Note: This assumes the scaler was fitted on the raw columns present in df_processing
            if scaler:
                self.signals.progress.emit("Applying feature scaling to raw data...")
                scaled_values = scaler.transform(df_processing.values)
                # Reconstruct dataframe to maintain column names for the selection step
                df_processing = pd.DataFrame(scaled_values, columns=df_processing.columns)

            # 4. Select Features and Handle Missing Columns
            # We do this after scaling so we can pull the already-scaled columns by name
            ...
            # Filter and ensure correct column order for the model
...
class PredictionScenarioWV(QWidget, ResetMixin):
